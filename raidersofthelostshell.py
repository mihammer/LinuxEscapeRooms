...
def intro():
    ...
    slow(textwrap.dedent("""
        ...
        Type 'hint' for a clue. Type 'skip' after 3 misses to continue (no key earned).
    """))
...
def challenge_ls(inventory):
    ...
    ask_shell("traps> ", {"ls -a"},
              "`ls -a` includes hidden files (those beginning with a dot).",
              inventory, key_name="Glass Key",
              hint="Use `ls` with a flag that includes dotfiles.")
...
def challenge_ps(inventory):
    ...
    ask_shell("fork> ", {"ps"},
              ...
              inventory, key_name="Iron Key",
              hint="Two letters. Short for 'process status'.")
...
def challenge_kernel(inventory):
    ...
    ask_shell("kernel> ", {"uname -r", "uname -a"},
              "`uname -r` shows kernel version.",
              inventory, key_name="Steel Key",
              hint="Try a command that starts with 'uname'.")
...
def challenge_sshd(inventory):
    ...
    "These commands verify if the SSH service is running.",
    inventory, key_name="Obsidian Key",
    hint="Try `systemctl`, `service`, or `ps` to check services.")
...
def challenge_tail(inventory):
    ...
    ask_shell("log> ",
              {"tail /var/log/syslog", "tail -n 10 /var/log/syslog", "tail -10 /var/log/syslog"},
              "`tail` lets you read the last lines of a file.",
...
def challenge_lslt(inventory):
    ...
    ask_shell("tomb> ", {"ls -lt", "ls -t"},
              "`ls -lt` sorts files by modification time.",
              inventory, key_name="Marble Key",
              hint="Try `ls` with flags that sort by time.")
...
def challenge_who(inventory):
    ...
    ask_shell("rival> ", {"who", "w"},
              "`who` shows logged in users and their source.",
              inventory, key_name="Jade Key",
              hint="Use a simple command that asks 'who' is online.")
...
def challenge_cat(inventory):
    ...
    ask_shell("shell> ", {"cat script.sh", "less script.sh", "nano script.sh"},
              "Use `cat`, `less`, or `nano` to read before executing.",
              inventory, key_name="Golden Key",
              hint="You can display file content or open it in a viewer/editor.")
